...
# ------------------------------------------------------------------
# Wave height calculation (extracted from shader)
# ------------------------------------------------------------------
...
func _mat2_mult(m: Array, v: Vector2) -> Vector2:
	return Vector2(
		m[0][0] * v.x + m[0][1] * v.y,
		m[1][0] * v.x + m[1][1] * v.y
	)
...
func get_wave_height(world_pos: Vector2) -> float:
	...
	for i in range(ITER_GEOMETRY):
		var d: float = _sea_octave((uv + Vector2(current_time * sea_speed, current_time * sea_speed)) * freq, choppy)
		d += _sea_octave((uv - Vector2(current_time * sea_speed, current_time * sea_speed)) * freq, choppy)
		h += d * amp
		uv = _mat2_mult(OCTAVE_M, uv)
		freq *= 1.9
		amp *= 0.22
